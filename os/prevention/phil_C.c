...
void Take_R1(Lock *L1, CondVar *C1){ //R1 젓가락을 가져오기 위한 함수
	...
	while(Load("R1.txt") == 0){ //만약 자원이 없으면 대기 상태로 들어간다.
...
void Take_R3(Lock *L3, CondVar *C3){
	...
	while(Load("R3.txt")==0){//만약 자원이 없으면 대기 상태로 들어간다.
...
void Put_R3(Lock *L3, CondVar *C3){ //R3 젓가락을 놓기 위한 함수 식사를 다하였을 때 수행하게된다.
	...
	Release(L3); //Put_R3을 수행하였으므로 L3을 Release 시켜준다.
}

void Phil_C(Lock *L1, CondVar *C1,Lock *L3, CondVar *C3){//C 철학자가 밥을 먹기 위해 수행하는 함수

...
void Take_R2(Lock *L2, CondVar *C2){ //R2 젓가락을 가져오기 위한 함수
	...
	while(Load("R2.txt")==0){ //만약자원이 없다면 대기한다.
...
void Take_R3(Lock *L3, CondVar *C3){//R3 젓가락을 가져오기 위한 함수
	Acquire(L3);//L3에 관하여 acquire함으로써 mutual exclisive 하게 해준다.
	while(Load("R3.txt")==0){ //만약 자원이 없으면 대기상태에 들어간다.
		printf("message: %d가 R3을 기다림 \n",getpid());
		Wait(C3,L3); //wait 상태로 들어간다.
		printf("message: %d가 R3을 기다리다가 깨어남 \n",getpid());
	}
	Store("R3.txt",0); //젓가락을 얻었으므로 R3.txt에 0을 저장시켜준다
	printf("message: %d가 R3을 가져옴 \n",getpid());

	Release(L3); //Take_R3을 수행하였으므로 L3을 Release 시켜준다.
}
...
void Put_R2(Lock *L2, CondVar *C2){ //R2 젓가락을 놓기 위한 함수 식사를 다하였을 때 수행하게된다.
	...
	Release(L2); //Put_R2을 수행하였으므로 L1을 Release 시켜준다.
}

void Put_R3(Lock *L3, CondVar *C3){ //R3 젓가락을 놓기 위한 함수 식사를 다하였을 때 수행하게된다.
	Acquire(L3); //L1에 관하여 acquire함으로써 mutual exclisive 하게 해준다.
	Store("R3.txt",1); //젓가락을 놓았으므로 R3.txt 에 1을 저장시켜준다.
	Signal(C3); //위에서 처럼 자원을 젓가락을 놓았으므로 전체 자원의 개수를 증가시켜준다.
	printf("message: %d가 R3을 내려둠 \n",getpid());
	Release(L3); //Put_R3을 수행하였으므로 L1을 Release 시켜준다.
}

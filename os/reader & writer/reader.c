...
int main(int argc, char * argv[]) {
  ...
  pid_t pid; //pid 값을 받기 위한 변수
   ...
   while((AW+WW)>0){ //만약 Aactive Writer나 Wating Writer이 존재한다면 wait 해준다.
      add("WR.txt",1);
      Wait(&okToRead,&lock);
      sub("WR.txt",1);
      AW = Load("AW.txt");
   ...
   add("AR.txt",1); //한번 읽었기 때문에 더해준다. /* 화일에서 읽어서 1 더하기 */

   sleep(atoi(argv[2]));//두번째 슬립인자를 받는다.
   ...
   if( AR == 0 && WW > 0 ){ //Active reader가 없고 Wating Writer가 있다면 signal을 통해 활성화 시켜준다. 
  	Signal(&okToWrite);
   }
   printf("process %d leaving critical section\n", pid);
   Release(&lock);   // lock.Release()

...
void Take_R1(Lock *L1,CondVar *C1) { //R1 젓가락을 가져오기 위한 함수
	...
	while ((Load("R1.txt") == 0) ||((Load("banker_resource.txt")<2) && (Load("state.txt")==0))) {
  ...
  sub("banker_resource.txt",1); //젓가락을 A 철학자가 얻음으로 써 전체 resource는 감소한다.
...
void Take_R2(Lock *L1,CondVar *C2) { //R2 젓가락을 가져오기 위한 함수
  ...
  sub("banker_resource.txt",1); //젓가락을 A 철학자가 얻음으로 써 전체 resource는 감소한다.
	add("state.txt",1); //이때 A철학자가 젓가락을 두개 얻어서 밥을 먹을 수 있으므로 state.txt에 1을 저장하여 식사를 하였음을 나타낸다.
...
void Phil_A(Lock *L1,CondVar *C1, CondVar *C2) { //A 철학자가 밥을 먹기 위해 수행하는 함수

...
void Take_R2(Lock *L1,CondVar *C2) { //R2 젓가락을 가져오기 위한 함수
	Acquire(L1); //L2에 관하여 acquire함으로써 mutual exclisive 하게 해준다.
	while ((Load("R2.txt") == 0) || ((Load("banker_resource.txt")<2)&& (Load("state.txt")==0))) {
		//만약 자원이 없거나, 남은 자원이 2개 보다 작고 어떤 철학자도 식사를 하지않는다면 이때 wait 상태에 들어가야한다.
		printf("message: %d R2을 기다림 \n",getpid());
		Wait(C2,L1); //wait 상태로 들어간다.
	  printf("message: %d R2을 기다리다가 깨어남 \n",getpid());
	}
	Store("R2.txt", 0); //젓가락을 얻었으므로 R2.txt에 0을 저장시켜준다.
  sub("banker_resource.txt",1); //젓가락을 B 철학자가 얻음으로 써 전체 resource는 감소한다.
	printf("message: %d R2을 얻음 \n",getpid());
	Release(L1);  //Take_R2을 수행하였으므로 L1을 Release 시켜준다.
}
...
void Take_R3(Lock *L1,CondVar *C3) { //R3 젓가락을 가져오기 위한 함수
  ...
  sub("banker_resource.txt",1); //젓가락을 B 철학자가 얻음으로 써 전체 resource는 감소한다.
	add("state.txt",1); //이때 B 철학자가 젓가락을 두개 얻어서 밥을 먹을 수 있으므로 state.txt에 1을 저장하여 식사를 하였음을 나타낸다.
	printf("message: %d R3을 얻음 \n",getpid());
	Release(L1); //Take_R3을 수행하였으므로 L1을 Release 시켜준다.
}

void Put_R2(Lock *L1,CondVar *C2) {  //R2 젓가락을 놓기 위한 함수 식사를 다하였을 때 수행하게된다.
	Acquire(L1); //L1에 관하여 acquire함으로써 mutual exclisive 하게 해준다.
  ...
  Signal(C2); //queueR2에 대기중인 철학자를 깨워준다.
  printf("message: %d R2을 내려놓는다.\n",getpid());
	Release(L1); //Put_R2을 수행하였으므로 L1을 Release 시켜준다.
}
...
void Phil_B(Lock *L1,CondVar *C2,CondVar *C3) {//B 철학자가 밥을 먹기 위해 수행하는 함수
 	Take_R2(L1, C2); //R2를 가져오기 위한 함수
...
int main(int argc, char * argv[]) {
  ...
  key_t semkey = 0x100; //semkey값을 지정한다.
	key_t semkey_chopsticks1 = 0x200; //semkey_chopsticks2 값을 지정한다.

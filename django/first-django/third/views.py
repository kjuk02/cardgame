...
def list(request):
    restaruants = Restaurant.objects.all().annotate(reviews_count=Count('review')).annotate(average_point=Avg('review__point'))
    paginator = Paginator(restaruants, 5)
    page = request.GET.get('page')
    items = paginator.get_page(page)
    last_bar_number = int(paginator.num_pages - 1)

    context = {
        'restaurants': items,
        'last_page': last_bar_number
    }
...
def update(request):
    if request.method == 'POST' and 'id' in request.POST:
        ...
        #item = Restaurant.obje cts.get(pk=request.GET.get('id'))
